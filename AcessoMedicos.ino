...
bool verificarEntradaRegistrada(String uid) {
  ...
  if (file) {
    while (file.available()) {
      ...
      if (linha.indexOf("UID: " + uid) >= 0 && linha.indexOf("Entrada") >= 0) {
        // Verifica se há uma entrada sem saída correspondente
        if (linha.indexOf("Saída") == -1) {
          file.close();
          return true; // Entrada registrada sem saída
        }
      }
    }
    file.close();
  }
  return false; // Nenhuma entrada registrada sem saída
}
